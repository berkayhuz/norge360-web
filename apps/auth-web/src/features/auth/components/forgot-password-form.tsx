...
export function ForgotPasswordForm() {
  ...
  function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const parsed = forgotPasswordSchema.safeParse({ email: getFormString(new FormData(event.currentTarget), "email") })
    if (!parsed.success) {
    ...
    startTransition(async () => {
      try {
        ...
        setMessage({ text: "Hesap bulunursa sifre sifirlama baglantisi gonderilecektir.", tone: "success" })
      } catch (error) {
  ...
  return (
    <form className="grid gap-4" noValidate onSubmit={onSubmit}>
      ...
      <SubmitButton disabled={pending}>{pending ? "Gonderiliyor..." : "Sifirlama baglantisi gonder"}</SubmitButton>
    </form>
  )
}
