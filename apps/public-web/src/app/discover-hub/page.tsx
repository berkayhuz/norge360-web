      ...
      <main className="mx-auto flex w-full flex-1 flex-col py-5 space-y-4">
        ...
        {hub ? (
          <div className="grid gap-5 lg:grid-cols-1">
            <DiscoverSection items={items} title={t("discovery.sections.suggested")} />
          </div>
        ) : null}
...
function DiscoverSection({ items }: { items: DiscoverUser[]; title: string }) {
  const t = useTranslations("public-web");
      ...
      <CardContent className="space-y-4 p-0">
        {items.map((item) => (
          <div className="flex items-start gap-3" key={item.profileId}>
            <Link href={`/${item.username}`} className="block w-full">
              <div className="px-4 py-3 flex items-start gap-3 transition-colors hover:bg-muted/60 dark:hover:bg-muted/25">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {item.bio ? <p className="mt-1 line-clamp-2 text-xs text-foreground">{item.bio}</p> : null}
                </div>
